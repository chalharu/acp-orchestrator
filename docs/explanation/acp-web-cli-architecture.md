...
flowchart LR
    ...
    subgraph infrastructure["Infrastructure Layer"]
        ...
        workspaceStore["WorkspaceRepository<br/>owner / upstream / workspace CRUD"]
...
| `WorkspaceStorePort` | workspace owner、upstream、workspace CRUD の保存/読出し | `WorkspaceRepository` |
...
- `CreateWorkspace`
  - owner に紐づく workspace metadata を作り、upstream policy を検証する
- `UpdateWorkspace`
...
backend が持ちます。
新規 session create は `POST /api/v1/workspaces/{workspace_id}/sessions` に限定します。
Web / CLI は workspace を作成または選択した後にだけ session を開始します。
