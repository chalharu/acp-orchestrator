...
この sprint で確認できる範囲は agent process の起動・環境注入・cleanup までである。
prompt transport は従来どおり mock / external ACP server を使う。
...
`ACP_CHECKOUT_RELPATH`、`ACP_AGENT_LAUNCH_MODE` である。書き込み後は短時間 sleep するだけでよい。
