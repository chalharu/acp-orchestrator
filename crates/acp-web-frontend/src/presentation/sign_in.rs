...
pub fn SignInPage() -> impl IntoView {
            ...
            match api::auth_status().await {
                Ok(status) => match home_route_target(&status) {
                    ...
                    HomeRouteTarget::Register => {
                        let _ = navigate_to("/app/register/");
                    }
                    HomeRouteTarget::SignIn => loading.set(false),
                },
                Err(message) => {
                    loading.set(false);
    ...
    let on_submit = move |event: web_sys::SubmitEvent| {
        ...
        leptos::task::spawn_local(async move {
            match api::sign_in(&username_value, &password_value).await {
                Ok(_) => {
                    let _ = navigate_to("/app/");
                }
                Err(message) => {
        ...
        <main class="app-shell app-shell--home">
            ...
            <section class="panel account-panel">
                ...
                <Show
                    ...
                    fallback=|| view! { <p class="muted">"Checking sign-in status…"</p> }
                >
