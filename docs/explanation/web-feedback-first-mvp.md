...
| 実装済み | permission / cancel | tool activity panel 内の pending permission UI、approve / deny、panel 側 cancel、返信待ち中の composer 側 cancel を実装 | bundled mock の `verify permission` / `verify cancel` を人手で確認できること |
...
| 実装済み | slash / tool panel | composer で `/` 入力中に即時 slash palette を出し、tool activity panel で status / help / pending permission をまとめて扱う。topbar には connection / worker badge を出す | browser でも `/help` `/cancel` `/approve` `/deny` を使いながら、permission と状態変化を transcript 外の focused area で確認できること |
...
- `chat-dock`: tool activity panel と composer

session 一覧については narrow screen でも壊れないよう、toggle で開閉するだけの単純な
sidebar に留めています。slash palette と tool activity panel は追加しましたが、
...
| tool activity panel | pending permission や slash help / status を扱う | permission resolution API + slash completions API | session owner に限って permission を解決し、slash help は backend completion catalog を再利用する |
| cancel action | 実行中 turn を止める | cancel API | tool activity panel 側と composer 側の両方に cancel affordance がある |
...
- pending permission は tool activity panel で approve / deny / cancel できる
- 返信待ち中は composer footer に cancel を出す
- composer で `/` を打つと slash palette が即時に出て、候補を keyboard / click で補完できる
- `/help` は browser 内の tool activity panel に available commands を出す
