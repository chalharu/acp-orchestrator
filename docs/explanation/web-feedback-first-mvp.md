...
| 実装済み | slash / activity | composer で `/` 入力中に即時 slash palette を出し、chat area に recent activity と pending permission を出す。topbar には connection / worker badge を出す | browser でも `/help` `/cancel` `/approve` `/deny` を使いながら、permission と状態変化を chat area 内で確認できること |
...
- `chat-body`: 仮想スクロール付き transcript と inline activity
- `chat-dock`: composer

session 一覧については narrow screen でも壊れないよう、toggle で開閉するだけの単純な
sidebar に留めています。slash palette と inline activity は追加しましたが、
