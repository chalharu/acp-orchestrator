...
async fn direct_mock_server_accepts_tcp_connections() -> Result<()> {
    ...
    acp_web::support::http::wait_for_tcp_connect(&address, 50, Duration::from_millis(50))
        .await
...
async fn direct_backend_server_reports_health() -> Result<()> {
    ...
    let client = acp_web::support::http::build_http_client_for_url(&base_url, None)
        .context("building test client")?;

    acp_web::support::http::wait_for_health(&client, &base_url, 50, Duration::from_millis(50))
        .await
        .map_err(|error| anyhow::anyhow!("{error}"))?;
