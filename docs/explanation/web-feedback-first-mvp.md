...
| 実装済み | session management | sidebar から session title の rename / delete を行える。title は最初の user prompt から自動で付き、same-tab の draft は session ごとに `sessionStorage` へ保持する。backend 都合で closed になった session は list に残り、chat area で ended と表示する | browser から rename / delete / draft persistence を確認でき、closed session が read-only だと分かること |
| 実装済み | slash / activity | composer で `/` 入力中に小さな floating slash palette を出し、chat area に recent activity と pending permission を出す。topbar には connection / worker badge を出す | browser でも `/help` `/cancel` `/approve` `/deny` を使いながら、permission と状態変化を chat area 内で確認できること |
...
- `chat-body`: 仮想スクロール付き transcript と inline activity feed
- `chat-dock`: composer

session 一覧については narrow screen でも壊れないよう、toggle で開閉するだけの単純な
sidebar に留めています。floating slash palette と inline activity は追加しましたが、
...
| chat activity | pending permission や status を扱う | permission resolution API + slash completions API | session owner に限って permission を解決し、slash help は backend completion catalog を再利用する |
...
- session row から delete でき、current session を消した場合は別 session または `/app/` へ移る
...
- composer で `/` を打つと小さな floating slash palette が出て、候補を keyboard / click で補完できる
- `/help` は browser 内の activity 履歴に available commands を出す
