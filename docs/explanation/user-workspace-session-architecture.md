...
**target design の詳細設計**です。既存の target architecture は Web / CLI / backend の
責務分離を主題にしていますが、ここではその中でも
**`User -> Workspace -> Session` の所有構造、永続メタデータ、Git clone、session cleanup**
を具体化します。

- これは **現状実装の仕様書**ではなく、今後の実装判断を揃えるための設計書です
- 既存の target architecture にある owner check、backend 主導の worker 管理、
  `SQLite + append-only event log` の方向性はそのまま継承します
- feedback-first な CLI / Web の文書は引き続き「現在の出し方」を扱い、この文書は
  それらの次の土台を定義します
...
- `users`
  - `user_id`, `principal_kind`, `principal_subject`, `default_workspace_id`,
    `created_at`, `last_seen_at`
- `workspaces`
  - `workspace_id`, `owner_user_id`, `name`, `upstream_url`, `default_ref`,
    `status`, `created_at`, `updated_at`, `deleted_at`
- `sessions`
  - `session_id`, `workspace_id`, `owner_user_id`, `title`, `status`,
    `checkout_relpath`, `checkout_ref`, `checkout_commit_sha`, `failure_reason`,
    `detach_deadline_at`,
    `created_at`, `last_activity_at`, `closed_at`, `deleted_at`
...
delete が `closing` 中に到着した場合は、close を別経路でやり直さず、
進行中の worker stop / final snapshot / checkout cleanup をそのまま引き継いだうえで
owner-facing API からの不可視化と purge を `deleting` で続けます。
delete が `busy` / `canceling` 中に到着した場合は、いきなり hard-stop せず、まず cancel / drain を要求して
close 相当の final snapshot を確定させ、その後に `deleting` へ進めます。graceful path が timeout した時だけ
強制停止へフォールバックします。
...
`POST /api/v1/sessions` は migration 用の compatibility endpoint とし、
backend が user の default workspace を解決して内部的に
`POST /api/v1/workspaces/{workspace_id}/sessions` へ委譲します。
この path は、後述の bootstrap / backfill で default workspace を準備した段階でだけ
有効化します。default workspace がまだ存在しない場合は、backend が勝手に曖昧な workspace を
作らず、`409 workspace_required` を返して workspace bootstrap へ進ませます。
default workspace の正本は `users.default_workspace_id` とし、null のまま active workspace が複数ある場合は
compatibility path を失敗させます。active workspace が 1 件しかない場合だけ、backend が
`default_workspace_id` を lazily 補完できます。
初期段階では `upstream_url` は create 時にだけ設定できる immutable field とし、
別 upstream へ切り替える場合は新しい workspace を作ります。
`default_ref` は mutable ですが、branch / tag / commit selector として妥当な値だけを受け付け、
先頭 `-`、制御文字、曖昧な revision expression を拒否します。Git 実行時は shell 展開せず、
構造化された引数として渡します。
...
ここで workspace delete を許す terminal state は `closed` / `deleting` / `deleted` とし、
`failed` / `closing` を含むそれ以外の状態はすべて block 対象にします。`deleting` は
owner-facing API では見えませんが、purge が既に始まっているため workspace delete の blocker にはしません。
...
`SessionStateStorePort`、`WorkerLifecyclePort` を組み合わせて使い、
workspace 所有情報の解決、checkout materialization、runtime metadata 保存、worker 制御を分けます。
