...
現在は
**launcher + minimal chat shell + slash/chat activity + permission/cancel + session 再開の一部 + 仮想スクロール**
...
| 実装済み | permission / cancel | chat area 内の pending permission UI、approve / deny、chat area 側 cancel、返信待ち中の composer 側 cancel を実装 | bundled mock の `verify permission` / `verify cancel` を人手で確認できること |
...
| 実装済み | session list UI | session route shell に collapsible な左 sidebar を追加し、owner-scoped session list を backend の並び順そのままで compact な 1 行 row で表示する。current / closed は badge ではなく styling で見分ける | 現在の session と closed session を sidebar で見分けながら、別 session へ移動できること |
| 実装済み | session management | sidebar から session title の rename / delete を行える。title は最初の user prompt から自動で付き、same-tab の draft は session ごとに `sessionStorage` へ保持する。backend 都合で closed になった session は list に残り、chat area で ended と表示する。最後の 1 件を delete した場合も fresh な new chat へ戻る | browser から rename / delete / draft persistence を確認でき、closed session が read-only だと分かること |
| 実装済み | slash / activity | composer で `/` 入力中に小さな floating slash palette を出し、browser では `/help` のみを補完する。recent activity は transcript の status entry として混ぜ、pending permission と cancel は chat area / composer の visible control で扱う。topbar には connection / worker badge を出す | browser でも minimal な `/help` と visible action control を使いながら、permission と状態変化を chat area 内で確認できること |
...
- `session-sidebar`: `New session` entry point、owned session list、inline rename / delete、clearer show / hide affordance
- `chat-topbar`: sidebar toggle、connection / worker badge、error banner
- `chat-body`: 仮想スクロール付き transcript と pending permission
- `chat-dock`: composer

session 一覧については narrow screen でも壊れないよう、toggle で開閉するだけの単純な
sidebar に留めています。floating slash palette と transcript 内 activity は追加しましたが、
最終 multi-pane shell まではまだ広げていません。
...
```sh
# frontend 単体の主な確認
cargo fmt --manifest-path crates/acp-web-frontend/Cargo.toml -- --check
...
| session rename | session title を付け直す | rename API | session owner に限って inline rename できる |
| session delete | 不要な session を消す | delete API | owner-scoped に hard delete する。current session を消した場合は次の session または `/app/` へ移る |
| session close | active session を終了する | close API | browser UI の主操作には出さないが、backend lifecycle により closed session が残ることはある |
...
- session route には collapsible な左 sidebar があり、`New session` と compact な owned session list を出す
- session list は backend が返した recent order をそのまま表示し、session を開くだけでは並び替えない
- session title は既定では `New chat` で、最初の user prompt から自動で付き、必要なら inline rename できる
- current session は badge ではなく row の styling で見分ける
- session row から delete でき、最後の current session を消した場合も stale session を再利用せず fresh な new chat へ移る
- same-tab の composer draft は session ごとに保持し、session switch や route reload でも戻る
...
- pending permission は chat area で approve / deny / cancel できる
- 返信待ち中は composer footer に cancel を出す
- composer で `/` を打つと小さな floating slash palette が出て、browser では `/help` だけを keyboard / click で補完できる
- `/help` は transcript 内の status entry に available commands を出し、送信後は次の入力に戻れるよう composer focus を回復する
- topbar に connection / worker state badge を出す
- stream や session load の失敗は banner と composer status text に出す
- composer は `Enter` で送信し、`Shift+Enter` で改行する
- closed session は snapshot / transcript の表示だけを行い、SSE を張らない read-only 扱いにする。composer には ended message を出す
- action 失敗と connection 失敗は別管理にし、action error は次の action を始めるまで維持する
- full multi-pane shell はまだ出していない
...
+----------------------+-------------------------------------------+
| Show sessions        | Bundled mock ready.                       |
| + New session        | Try `verify permission` or `verify cancel`.|
| > README の要点... ✎✕ |                                           |
|   Old exploration    | [user] README の要点を教えて                    |
...
- browser UI からの session rename / delete
- session ごとの composer draft persistence
- prompt 送信
- live event の購読
...
- dedicated dashboard / overview route
- full activity rail outside the chat body
