...
- 開発初期のエンドユーザー確認は install 済み CLI ではなく、repo root から `cargo run --bin acp -- ...` で行う
...
| 1. 最小会話導線 | とにかく会話できる状態を早く出す | session 作成、prompt 送信、SSE 受信、終了処理、line-oriented CLI を実装する | repo root から `cargo run --bin acp -- chat --new` を実行し、会話を 1 往復以上できること |
| 2. permission / cancel | 実運用に近い最小制御を足す | permission request 表示、approve / deny、実行中 turn の cancel を実装する | Agent からの permission 要求に CLI で応答できること |
| 3. session 継続 | 再利用と途中復帰を可能にする | recent-session list / attach、history 取得、snapshot 表示を実装する | CLI を再起動しても既存 session に戻れること |
...
ユーザーがそのまま試せること**を優先します。そのため、最初の確認導線は
`cargo run --bin acp -- <subcommand>` に固定します。

- 新規会話開始: `cargo run --bin acp -- chat --new`
- 既存 session へ再 attach: `cargo run --bin acp -- chat --session <id>`
- recent session の確認: `cargo run --bin acp -- session list`

この文書で `acp ...` と書く箇所は、将来の install 後も維持したい**論理コマンド名**です。
開発初期の実行では、それを `cargo run --bin acp -- ...` に読み替えます。実装時も
Cargo の binary target 名は `acp` にそろえ、`cargo run` と install 後の command
surface がずれないようにします。
...
| `acp chat --session <id>` | 既存 session に attach して会話を再開する | `GET /api/v1/sessions/{id}` + `GET /api/v1/sessions/{id}/history` + `GET /api/v1/sessions/{id}/events` |
| `acp session list` | attach 候補を一覧する | backend API は増やさず、CLI が保持する recent-session cache を表示する |
...
`acp session list` は backend contract を増やさず、CLI 自身が「自分で作成または attach した
session ID」の recent cache を保持する前提にします。これにより、target architecture の
session-scoped API を崩さずに復帰導線を先に提供できます。
