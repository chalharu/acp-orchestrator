...
- Web の `/` は、現在の「即 session を作る route」から
  **workspace の明示作成または選択** に寄せる
- 初回 `/` では default workspace を自動作成しない
- Web の canonical deep link は
  `/workspaces/{workspace_id}/sessions/{session_id}` を推奨する
- 既存の `/app/` と `/app/sessions/{session_id}` は、それぞれ `/` と
  `/workspaces/{workspace_id}/sessions/{session_id}` への compatibility redirect を持てる
...
6. Web の `/` を workspace bootstrap へ寄せ、旧 `/app/*` frontend route は
   compatibility redirect へ移し、CLI に workspace command を追加する
...
1. Web の初回 `/` は default workspace を自動作成せず、workspace の明示作成または選択を求める
   - 既存の `/app/*` frontend route は compatibility redirect で扱う
...
- `/` を immediate session creation から workspace bootstrap / select に変更する
- workspace-scoped deep link を追加する
- 旧 `/app/` と `/app/sessions/{session_id}` の compatibility redirect を維持する
- 既存 Web shell に最小限の workspace 作成 / 選択 UI を追加する
- 完了条件は、Web から workspace を作成または選択し、その配下で session を開始・遷移できることである

### 13.6 このスライス順を支える前提

- `/` では default workspace を自動作成しない
