...
説明する Explanation です。既存実装の挙動を列挙する文書ではなく、
ACP Orchestrator として切り出す再設計の到達点を整理するための設計書です。
この repo では、まず target architecture を基準文書として管理します。
...
- Clean Architecture
- Tokio
- 既存実装で使っている process 管理・state 取扱いパターンを参考にする
...
flowchart LR
    ...
    subgraph application["Application Layer"]
        ...
        sessionCommands["Session Command Use Cases<br/>CreateSession / RestartSession / CloseSession / DeleteSession"]
        ...
        maintenanceUseCases["Background Maintenance Use Cases<br/>ResumeWorkspaceDelete / ResumeSessionCleanup / RetentionPurge"]
    end
    ...
    subgraph infrastructure["Infrastructure Layer"]
        ...
        workspaceFs["WorkspaceFileAdapter<br/>dirfd-scoped workspace read / write"]
        ...
        wasmAssets["WasmAssetProvider<br/>root shell + /assets/* delivery"]
    end
    ...
    maintenanceUseCases --> workspaceStorePort
    maintenanceUseCases --> sessionStorePort
    maintenanceUseCases --> checkoutPort
    maintenanceUseCases --> eventLogPort

    workspaceCommands --> workspaceStorePort
    workspaceCommands --> sessionStorePort
    workspaceCommands --> checkoutPort
...
sequenceDiagram
    ...
    Backend->>Backend: credential lease revoke
...
sequenceDiagram
    ...
    alt file read / write
        ...
        Backend->>WorkspaceFS: dirfd containment check + read / write
...
`Creating` は fresh create では `provisioning / cloning / starting` を束ねる。
`Restartable` からの restart では既存 checkout を再利用するため、`Creating` の中で
`starting` だけを通り `cloning` は行いません。
...
stateDiagram-v2
    ...
    Creating --> Failed: initial create failed
    Creating --> Restartable: crash recovery / restart start failed with intact checkout
    ...
    Canceling --> Detached: disconnect during cancel
    Canceling --> Closing: explicit close queued; after cancel ack
    ...
    Canceling --> Restartable: backend restart / live worker lost
    Ready --> Restartable: backend restart / live worker lost
    Streaming --> Restartable: backend restart / live worker lost
    Busy --> Restartable: backend restart after reconciliation
    Detached --> Restartable: backend restart / live worker lost

    Ready --> Closing: explicit close before first stream
    Creating --> Closing: parent workspace deleting during recovery
    ...
    Restartable --> Creating: explicit restart
    Restartable --> Closing: explicit close or restart TTL expired
    Restartable --> Failed: deterministic corruption detected
    ...
    Restartable --> Deleting: explicit delete
    Closed --> Deleting: explicit delete or retention expiry
    Failed --> Deleting: explicit delete or workspace cleanup
...
`Restartable` への recovery edge は、親 workspace が `active` の場合だけ適用します。
workspace が `deleting` / `deleted` の場合は child doc 6.6 のとおり `Closing` / `Deleting` を優先します。
`Busy -> Restartable` の reconciliation では partial turn を interrupted として seal し、
credential を新規取得せずに metadata 整合だけを確定します。
...
- `UpdateWorkspace`
  ...
  - `upstream_url` と managed credential reference (`credential_reference_id`) は phase 1 では immutable とする
- `ListOwnedWorkspaces`
  - owner に見せてよい workspace 一覧と default workspace metadata を返す
- `GetWorkspace`
  - 1 つの workspace 詳細を owner check 付きで返す
...
- `CreateSession`
  ...
  - 初回 snapshot を準備する
- `RestartSession`
  - `restartable` session の checkout / transcript / ownership を検証する
  - managed credential reference (`credential_reference_id`) が still valid かを再検証する
  - security-critical な `.git/config` を再適用し、危険な override を無効化する
  - Agent が `loadSession` を持つなら既存 ACP session の再構成を試みる
  - 非対応なら新しい ACP session を同じ FrontendSession に再束縛する
  - restart 失敗時は session を `Restartable` のまま残す
- `CloseSession`
  - session close を確定し、worker 停止、最終 snapshot 保存、後始末を指示する
  - `restartable` からの close では worker 停止を省略し、cleanup 確認後に `Closed` へ進める
- `DeleteSession`
  - session を owner-facing API から不可視化する
  - active turn 中なら cancel / drain と final snapshot capture を行う
  - purge queue と runtime cleanup を idempotent に進める
- `DeleteWorkspace`
...
- `WorkspaceFileAdapter`
  - `fs/read_text_file` / `fs/write_text_file` を child doc 5.2.1 の dirfd-based containment と
    allowlisted workspace root の範囲で実装する
  - symlink escape と `..` traversal を拒否する
- `TerminalBroker`
  - `terminal/create` / `output` / `wait_for_exit` / `kill` / `release` を session-scoped
    sandbox 上で実装する
  - working directory、child doc 6.3 と同じ closed env allowlist、resource limit、
    process group、timeout、output cap を管理する
  - approved binary set だけを execute 可能にし、raw system `git` は実行不可にする
...
backend が持ちます。
compatibility `POST /api/v1/sessions` は `default_workspace_id` を使って委譲する。
binding が無い場合は `409 workspace_required` を返し、
`PUT /api/v1/users/me/default-workspace` で明示的に再設定させる。
`PUT /api/v1/users/me/default-workspace` は owner の `active` workspace にだけ許可する。
default binding の解決、default workspace の更新、session row 作成は同じ owner transaction で行い、
workspace delete はその transaction と直列化する。

第一段階では `upstream_url` は workspace create 時にだけ受け付ける immutable field とする。
`PATCH /api/v1/workspaces/{workspace_id}` は rename と default ref 更新だけを扱う。
`upstream_url` や `credential_reference_id` を含む PATCH は `409 workspace_immutable_field` を返す。
`default_ref` は strict な ref validation を通した値だけを受け付ける。
Git には shell 展開せず、構造化引数で渡す。
phase 1 では redirect follow を無効化する。
将来 follow を許可する場合でも、各 redirect target に同じ scheme / host / resolved IP
...
compatibility `POST /api/v1/sessions` を維持している期間に default workspace を削除する場合は、
その owner の default binding をクリアします。
以後の compatibility `POST /api/v1/sessions` は、`PUT /api/v1/users/me/default-workspace` で
明示的に新しい default を束縛するまで
`409 workspace_required` を返します。
...
orchestrator の state 配下を基本候補にし、SQLite + append-only event log を
第一案とします。
...
capability に依存**します。`loadSession` 非対応の場合でも、backend 再起動後は
session を `Restartable` として残し、保持済み transcript / metadata / checkout を見せます。
owner が `POST /api/v1/sessions/{session_id}/restart` を呼ぶと、新しい ACP session を起動して
同じ FrontendSession へ再束縛する前提です。
credential 再検証や worker 起動が失敗した場合でも、session は `Restartable` のまま残します。
`POST /api/v1/sessions/{session_id}/restart` を `Restartable` 以外に対して呼んだ場合は
`409 session_not_restartable` を返します。
restartable TTL、grace extension、janitor 連携、deterministic corruption から `Failed` へ落とす条件は
child doc 6.6 / 6.7 を正本とします。
...
標準案では Axum が browser entry route の `/` と `/assets/*` 配下で Leptos CSR bundle を配信します。
将来 CDN / ingress 配信へ切り替える場合に備えて、API path と asset path は分けます。
既存の `/app/*` frontend route は migration 期間だけ compatibility redirect で扱います。
...
- session ID は CSPRNG で生成するが、access token ではない。すべての session-scoped API は
  owner check を再実行する
- `upstream_url` を受け取る write path は、scheme、credential 埋め込み、
  host / egress policy、optional な managed credential reference を検証する。
  第一段階では `https` だけを扱う。private repository では
  managed credential reference (`credential_reference_id`) を必須にする。
  `upstream_url` と `credential_reference_id` は immutable にし、workspace 更新での迂回を防ぐ
- push は clone / fetch と同列に扱わず、phase 1 では session owner の明示承認がある場合だけ
  workspace upstream 宛へ許可する。checkout branch から同名 upstream branch への単一 refspec だけを許可し、
  tag push / delete refspec / multi-ref push / force-push は拒否する。generic helper は read だけを扱い、
  承認済み push は orchestrator 実行の push wrapper だけが行い、one-shot write helper / lease も
  その wrapper にだけ渡す。write lease には絶対 TTL を持たせ、push subprocess は
  backend 管理の process group に閉じ込める。terminal 経由でも write credential は直接渡さず、結果は audit する
- Git stderr と credential 解決 error は scrub してから `failure_reason` や client-facing error に写像し、
  resolved credential 自体は URL / env / log に残さない
...
- 制限済み working directory、scrub 済み env、child doc 6.3 と同じ closed env allowlist、
  kernel-enforced filesystem sandbox を前提にする
- timeout、CPU / memory / output cap も前提にする
