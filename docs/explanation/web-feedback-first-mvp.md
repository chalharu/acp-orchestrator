...
| 実装済み | slash / activity | composer で `/` 入力中に小さな floating slash palette を出し、recent activity は transcript の status entry として混ぜ、pending permission は chat area 下部で扱う。topbar には connection / worker badge を出す | browser でも `/help` `/cancel` `/approve` `/deny` を使いながら、permission と状態変化を chat area 内で確認できること |
...
- `/help` は transcript 内の status entry に available commands を出す
