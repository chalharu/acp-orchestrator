...
理由は 2 通りで、legacy shared CWD 由来で最初から session-owned checkout を持たないか、
workspace delete が先にディスクを回収済みかのどちらかです。cleanup では両者を同じ意味で扱います。
...
`credential_reference_id` 自体は opaque な参照なので durable metadata に保存できます。
一方で、その参照先の実 credential は DB / event log / 通常ログへ保存しません。
backend 起動時には `<state-root>` 自体を `realpath` 相当で正規化し、absolute / canonical / non-root かつ
service user 所有であることを検証します。満たさない構成では backend を fail-fast します。
phase 1 の deployment target は case-sensitive な local filesystem に限り、起動時に mixed-case probe で
その前提も検証します。
`checkout_relpath` を読む側も、検証済み `<state-root>` の dirfd から
`openat2(..., RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS)`、またはそれと同等の
component-wise `openat(..., O_NOFOLLOW)` で単一 component ずつ解決し、配下に留まることを検証します。
`/` からの string-based canonicalize を再実行する実装は許可しません。
解決に失敗した session は scrubbed error を残して `failed` へ進めます。
checkout 作成 / cleanup / janitor の directory 操作も、検証済み `<state-root>` の dirfd から
`openat2(..., RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS)`、またはそれと同等の
component-wise `openat(..., O_NOFOLLOW)` を使って辿ります。
validated path string を別に開き直さず、session tree は同じ local filesystem device 内に作ります。
checkout 作成時は `fstat` で得た `st_dev` が state-root fd の `st_dev` と一致することも確認し、
不一致なら session を `failed` へ進めます。
`WorkspaceFileAdapter` と terminal の working directory は、state-root 配下であっても
さらに当該 session checkout root の dirfd に束縛し、cross-session path への到達を拒否します。
terminal subprocess はさらに kernel-enforced filesystem sandbox（Landlock、mount namespace、
同等の仕組み）に入れ、state-root の DB / events / 他 session checkout へ syscall レベルで到達できないようにします。
この sandbox では approved binary set だけを execute 可能にし、raw system `git` は
PATH 上にも絶対 path でも実行できないようにします。
approved binary set から `ln`、`install`、`git-lfs` など hardlink や Git extension 実行に使える tool は除外します。
checkout root 配下の任意 depth で、先頭 4 文字が `.git` の entry と `.git/` 配下は write 禁止にし、regular file / directory
以外の生成も許可しません。
この write 禁止は direct file operation に対するものであり、6.3 の shim によって許可された Git subcommand が
repo 内部を更新することまでは妨げません。
...
workspace delete は次の順で進めます。

1. owner 単位 transaction と workspace-level lock で対象を読み、`users.default_workspace_id` が対象を
   指す場合は clear する。delete に伴う自動再束縛は行わない
2. 同じ transaction の中で blocker を判定し、`closed` / `deleting` / `deleted` 以外の session が
   残る間は `409 workspace_has_active_sessions` で止める
3. blocker が無ければ `Workspace` を `deleting` にし、この時点以後の create / restart は
   `409 workspace_deleting` で拒否する
4. child ごとの session-level lock 下で先に `checkout_relpath` を `NULL` に tombstone し、
   workspace cleaner が disk cleanup の唯一の owner になった後で、lock の外で
   workspace directory tree 全体の recursive cleanup を実行する
5. 最後に workspace metadata と workspace-level binding を掃除し、`deleted` にする

ここで掃除するのは workspace metadata と binding であり、credential secret 自体ではありません。
workspace delete step 4 が tombstone した `checkout_relpath = NULL` は、session 側 cleanup に対する
「workspace cleaner が disk cleanup を引き取った」の正本シグナルです。session delete / close / janitor は
disk 操作前にこの値を読み直し、`NULL` または `ENOENT` を成功として扱います。
workspace-level lock は step 3 完了後に解放し、janitor が `deleting` workspace を再開する時は
step 4 から継続します。
`deleting` のまま止まった workspace は janitor が backoff 付きで retry します。
bounded retry budget を超えた場合も `deleted` へは進めず、scrubbed failure を記録して operator alert を出し、
`deleting` のまま明示介入を待ちます。
...
- clone / fetch / push には wall-clock timeout、転送量 / checkout size 上限、workspace / user ごとの
  disk budget を適用し、超過時は cleanup へ進める

clone 実行時の trust boundary も明示しておきます。

- clone 先は orchestrator 管理ディレクトリに固定する
- clone / fetch / push 実行時は `GIT_TERMINAL_PROMPT=0` を基本にし、`PATH`、orchestrator 管理の
  非 writable な空 `HOME`、`LANG`、`TMPDIR`、明示注入した `GIT_CONFIG_*` など必要最小限の
  closed allowlist だけを引き継ぐ。
  それ以外の `GIT_*` / `SSH_*` / `XDG_CONFIG_HOME` / proxy / loader / editor / TLS override 系の
  環境変数は渡さない
...
- checkout manager と push wrapper の Git 実行は、file-based overlay ではなく
  ...
  terminal capability には raw system `git` を露出せず、同じ closed env allowlist と `-c` 群、
  および `--git-dir=<checkout_root>/.git --work-tree=<checkout_root>` を再注入する
  orchestrator 管理の `git` shim だけを PATH に載せる。shim は shell から環境を引き継がず、
  毎回 fresh env を closed allowlist から組み立てる。invocation ごとに
  `GIT_CONFIG_COUNT` / `GIT_CONFIG_KEY_*` / `GIT_CONFIG_VALUE_*` / `GIT_CONFIG_PARAMETERS`、
  `GIT_EXEC_PATH`、`GIT_DIR`、`GIT_WORK_TREE`、`GIT_ASKPASS`、loader 系環境変数を除去し、
  user 指定 `-c` を拒否する。さらに phase 1 では shim を subcommand allowlist mode で動かし、
  `status`、`diff`、`log`、`show`、`add`、`commit`、read-only な `branch`、`checkout`、`switch`、
  `merge`、`rebase`、`reset`、`restore`、`stash`、`blame`、`grep`、`fetch`、`pull` だけを許可し、
  `config` の write form、`remote`、`credential`、`clone`、`submodule`、`worktree`、`push` などは拒否する。
  ...
  `--receive-pack`、`--ext-diff`、`--config-env`、`--git-dir`、`--work-tree`、`-C`、`--exec-path`、
  `--super-prefix` などの危険な global flag / option も拒否する。
  shim 実行直前にも `.git/config`、repository tree 配下の全 `.gitattributes`、
  `.git/info/attributes` を closed allowlist で再検証する。phase 1 の safe list は空とし、
  `filter=`、`diff=`、`merge=`、`textconv` の driver 参照が 1 つでもあれば fail closed する。
  `GIT_CONFIG_COUNT` は補助用途に留め、
  trust boundary としては使わない。少なくとも `GIT_CONFIG_NOSYSTEM=1`、`GIT_CONFIG_GLOBAL=/dev/null`、
...
- phase 1 では checkout manager / worker / terminal capability の Git 実行を同じ service uid で揃え、
  `safe.directory=*` は使わない。将来 split-uid にする場合も wildcard ではなく対象 checkout path だけを許可する
- phase 1 では repo-controlled な Git execution path（`include.*`、filter / smudge / clean driver、
  `diff.*.command` / `textconv`、`merge.*.driver`、repo-local credential helper、
  `diff.external`、`core.gitProxy`、`url.*.insteadOf` / `pushInsteadOf`、
  `http.proxy` / `http.<url>.proxy`、`http.sslCAInfo`、`http.sslCAPath`、`http.sslVerify`、
  `remote.*.promisor`、`alias.*`、`core.editor`、`core.pager`、`pager.*`、`sequence.editor`、
  `gpg.program`、`gpg.*.program`
  など）を許可しない。
  clone / start / restart のたびに `.git/config` と attribute 設定を **closed allowlist** で検証し、
  allowlist 外の key や危険な execution path が見つかったら fail closed する。
  さらに checkout manager / fetch / push など orchestrator 側の Git 実行直前にも同じ検証を再実行する。
...
- そのため初回 redirect を要求する provider は phase 1 の対象外とし、workspace create preflight で reject する
...
- private repository では `credential_reference_id` を orchestrator 管理の HTTPS credential helper へ解決し、
...
- push は generic helper から直接は許可しない。session owner が承認した時だけ、
  orchestrator 自身が target refspec と validated remote URL を検証したうえで、
  `remote.origin.url=<validated_url>` と空の `remote.origin.pushurl` を明示指定した
  push wrapper をその単一 invocation にだけ実行する。refspec 制限は parent architecture の 11.3 に従う
- credential lease は短命にし、少なくとも push 前と lease expiry 後の fetch 前には
  `credential_reference_id` を再解決する。`detached` 中は lease の再解決 / 延長を行わず、
  credential が必要な Git 操作は fail closed する
- file / terminal capability は 5.2.1 の containment と write-deny rule に従う。
  deny 判定と実際の write は同じ open file handle に対して行い、検証済み path string を
  後で開き直す実装は許可しない
...
`restartable` からの明示 close も同じ close path を使います。この場合は live worker が無いため、
worker stop をスキップして final snapshot / cleanup 確認から `closed` へ進めます。
`Busy` / `Canceling` 中の close request は、cancel / drain と final snapshot 確定の後に
`closing` へ進めます。
`Canceling` 中に close を受けた場合は `pending_close` intent を durable に残し、
crash recovery 後も再適用します。
`provisioning` / `cloning` / `starting` 中の close request は `409 session_not_closable` を返し、
abort が必要な場合は delete を使います。
...
`closed` への status 遷移は、final snapshot / event log の確定後にだけ行います。

つまり retention の対象は **履歴とメタデータ** であり、`repo/` や `runtime/` を残すことではありません。
通常の session close は `checkout_relpath` を `NULL` にしません。`checkout_relpath = NULL` は
workspace delete や migration で使う coordination marker であり、close 後の cleanup 済み判定は
`ENOENT` を含む実ディスク状態で行います。
...
delete が `provisioning` / `cloning` / `starting` 中に到着した場合は、in-flight clone や worker spawn を abort し、
...
session delete は disk 操作前に `checkout_relpath` を session lock 下で読み直し、`NULL` なら cleanup 済みとして
skip します。workspace delete など別経路が先に消していた場合も、`ENOENT` は成功として扱います。

### 6.6 crash recovery と janitor

backend 再起動時には durable state とディスク状態を照合します。

- durable metadata は `active` の中でも mid-turn を識別できるように
  `in_flight_turn_id` 相当の情報を保持し、crash recovery は `active -> restartable` の前に
  その turn を `interrupted` として event store へ seal します。`Busy` と `Canceling` の投影は
  この reconciliation を経由してから `restartable` / `closing` / `deleting` へ分岐します
...
- `workspace_id IS NULL` の coexistence session は workspace 依存の recovery に入れず、
  10 の migration rule に従う
- `restartable` へ補正する時点で、その session に紐づく credential lease は revoke し、
  revoke 失敗時は pending revocation を記録して restart 前に再試行する
- 親 workspace が `deleting` / `deleted` の場合は `restartable` へ戻さず、`closing` または
  `deleting` 側へ進める
- `restartable` session は read-only snapshot を返しつつ、`POST /api/v1/sessions/{session_id}/restart`
  を受け付ける
- restart は親 workspace が `active` であることを前提条件にする
- `restartable` の restart では既存 checkout を再利用する。restart 前には workspace の
  `credential_reference_id` がまだ有効かつ owner に許可されていることを再検証する
- ここでいう intact checkout の最低条件は、`.git/HEAD` が `checkout_commit_sha` へ解決でき、
  その commit object が存在し、軽量な connectivity check を通り、checkout root の `st_dev` が
  state-root fd と一致することです。単なる directory 存在だけでは足りません
- restart 前には clone / start と同じ `.git/config` / attribute 検証を再実行し、
...
- restart 前の re-hardening では workspace metadata 由来の安全な remote URL と process-level allowlist を
  再注入し、repo-controlled execution path を再び無効化する
- Agent が `loadSession` を持つなら
  同じ ACP session を復元し、持たないなら transcript と metadata を保持したまま新しい ACP session を
  同じ frontend session に再束縛する
- credential 再検証や worker 起動が失敗した場合、その試行で取得した credential lease は revoke したうえで、
  session は `restartable` のまま残し、restart request だけを失敗として返す
...
- ただし owner が deadline 後半で restart を再試行し、依存サービス側の一時障害で失敗した場合は、
  bounded grace を 1 回だけ付与できる
- restart 成功時は `restartable_deadline_at` を clear する
- 期限切れの `restartable` session は janitor が `closing` へ進め、checkout cleanup と
  retention path へ合流させる
- `detached` で worker が still live かつ親 workspace が `active` の場合だけ `detach_deadline_at` を見て TTL を再計算し、
  期限内なら timer を再開し、期限超過なら close path へ進める
- 親 workspace が `deleting` / `deleted` の `restartable` session は、TTL を待たずに janitor が
  `closing` へ進める
...
- 直後の cleanup は best-effort であり、`failed` session に partial checkout が残ることはある
...
- `deleting` workspace 配下の session に対する janitor の best-effort cleanup は disk 操作を行わず、
  workspace cleaner へ委譲する
...
既存 checkout を再利用し、credential 再検証、`.git/config` の再 hardening、worker 起動、
`initialize`、必要なら `loadSession` または `session/new` の順で進めます。
...
`GET /api/v1/sessions` は owner 全体の recent session を返し、各 item に `workspace_id` と
必要なら `workspace_name` を含めます。
compatibility `POST /api/v1/sessions`、workspace delete blocker、immutable field、default workspace
binding、race 制御の正本は parent architecture の 8.3 を参照します。
compatibility create の default binding 解決、`PUT /api/v1/users/me/default-workspace`、
workspace delete step 1、session row 作成は同じ owner transaction で直列化します。
...
4. `restartable`、`failed`、または長時間 `closing` の session が workspace delete を止める場合、
...
この hierarchy 導入で重要なのは、`Workspace` を durable metadata、`Session` を runtime 単位、
`WorkspaceFileAdapter` を session checkout root の file boundary として分離することです。

特に session create / restart では `WorkspaceStorePort`、`WorkspaceCheckoutPort`、
`SessionStateStorePort`、`WorkerLifecyclePort` を組み合わせ、workspace 所有情報の解決、
checkout materialization、runtime metadata 保存、worker 制御を混ぜずに扱います。
7.1 で述べた create / restart / delete に加え、detach、close、TTL expiry、reconnect、
worker crash recovery、workspace delete の blocker 判定も同じ直列化対象に含めます。
lock 順序は **owner-level serialization -> workspace-level lock -> session-level lock** に固定し、
session lock を保持したまま workspace lock を取りに行く実装は許可しません。
owner-level serialization は `users` row や default binding を触る操作
（compatibility create、`PUT /api/v1/users/me/default-workspace`、workspace delete step 1）にだけ使います。
...
これにより `deleting` へ遷移した workspace へ新規 session が滑り込む race を防げます。
長時間の clone / cleanup は lock の外で二相に進め、最初の transaction で `cloning` / `deleting` などの
中間状態だけを確定し、完了時に必要最小限の lock を取り直して結果を永続化します。
...
6. file / terminal capability は 5.2.1 の dirfd-based containment に従い、session checkout root の外へ出ず、
   worktree root で先頭 4 文字が `.git` の entry と `.git/` 配下への write は許可しない。
...
別の credential reference 経路で扱います。phase 1 では private HTTPS repository 用に
orchestrator 管理の `credential_reference_id` を受け付けます。
この参照は clone / fetch の read source に使い、provider 側でも write 不可な scope を必須にします。
read-only credential を発行できない provider は phase 1 の対象外です。
push の policy と lease rule の正本は parent architecture の 11.3 とし、
この文書では 6.3 の push wrapper 実装 detail だけを補います。
host / redirect / secondary fetch / gitconfig / hook / ambient credential の hardening は
6.3 の trust boundary に従います。
Git stderr や credential 解決 error は、そのまま `failure_reason` や SSE error に流しません。
secret を scrub した message だけを保存または返します。
credential は URL や環境変数ではなく、stdin-based credential helper から Git へ渡します。
`ssh` upstream は将来拡張候補として残しますが、phase 1 の対象外です。
...
3. owner-wide `GET /api/v1/sessions` と compatibility `POST /api/v1/sessions` を default workspace 経由で維持する
    ...
    - legacy shared CWD を使っていた session は `checkout_relpath = NULL` とし、close / delete cleanup では
      session-owned checkout が無いものとして扱う
    - backfilled session は step 5 まで旧 checkout path を保持してよく、cleanup / recovery は
      `workspace_id` から path を再計算せず `checkout_relpath` の正本を見る
    - coexistence 中に `workspace_id IS NULL` の session は、workspace 依存の recovery 分岐へ入れず、
      `active` / `detached` なら `closing`、それ以外は現状 state のまま backfill 完了または明示 close / delete を待つ
    ...
    - backfill と non-null 制約の有効化は別段階にし、`workspace_id IS NULL` の row が 0 件であることを
      確認してから制約を有効化する
    ...
    - step 2 の workspace bootstrap は create-if-not-exists で idempotent にし、step 3 は missing workspace を
      検出した時点で失敗として止める
...
5. session 起動を workspace 配下へ移し、checkout root を session 固有 path に切り替える
    ...
    - legacy shared CWD 自体は session-owned cleanup の対象外とし、janitor orphan scan から除外する
      operator-managed path として扱う
...
- step 4 と step 5 は forward-compatible に進め、旧 session は旧 path を維持することで rollback 時も読めるようにする
...
1. session 起動時に default branch HEAD だけでなく pinned ref を許可するか
   - ここでいう pinned ref は、workspace の `default_ref` とは別に session create request ごとに渡す
     checkout 対象です
   - 候補は branch 名、tag、commit SHA、あるいは `refs/heads/*` / `refs/tags/*` のような完全修飾 ref です
