...
この hierarchy 導入で重要なのは、`Workspace` を durable metadata として分離することです。
`Session` は runtime 単位とし、`WorkspaceFileAdapter` は session checkout root の file boundary として分離します。
...
例は compatibility create、`PUT /api/v1/users/me/default-workspace`、workspace delete step 1 です。
