...
永続 store 側では
`provisioning / cloning / starting / active / detached / closing / closed / deleting / failed / deleted`
...
binding が無くても、active workspace が 1 件だけなら lazily に補完できる。
それ以外は `409 workspace_required` を返す。
