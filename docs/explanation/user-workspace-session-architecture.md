...
| session 作成 API | `POST /api/v1/sessions` | `POST /api/v1/workspaces/{workspace_id}/sessions` を正規 route にする |
...
| エンティティ | 主責務 | 代表フィールド |
| --- | --- | --- |
| `User` | principal と owner record の対応付け | `user_id`, `principal_kind`, `principal_subject`, `created_at`, `last_seen_at` |
...
workspace delete は 4 段階で進めます。

1. active session の blocker を確認する
2. `Workspace` を `deleting` にする
3. disk cleanup の owner を workspace cleaner に切り替える
4. metadata を掃除して `deleted` にする
...
- step 4
  - workspace metadata を掃除する
  - 最後は `deleted` とする

補足ルールも明示しておきます。

- ここで掃除するのは workspace metadata であり、credential secret 自体ではない
...
- workspace-level lock は step 2 完了後に解放する
...
| `POST` | `/api/v1/workspaces/bootstrap` | loopback CLI 用 local workspace の idempotent bootstrap | bearer owner |
...
workspace delete blocker、immutable field、workspace-scoped create の正本は
parent architecture の 8.3 です。
新規 session create の正規 route は
`POST /api/v1/workspaces/{workspace_id}/sessions` だけです。
`POST /api/v1/sessions` のような owner-scoped create route は持ちません。
この文書で追加する API 契約は次の 3 点です。
...
2. `POST /api/v1/sessions/{session_id}/restart` は `restartable` にだけ有効で、それ以外は
   `409 session_not_restartable` を返す
3. `restartable`、`failed`、または長時間 `closing` の session が workspace delete を止める場合がある。
   owner は明示 close または `DELETE /api/v1/sessions/{session_id}` で unblock できる

### 7.2 session-scoped API

完全な route 一覧は parent architecture の 8.3 を正本とします。
ここでは session 起動後の live operation に使う session-scoped API を列挙します。

session 起動後の live 操作は、workspace 所有境界と runtime 操作を分離するため
`session_id` ベースの path を使います。
...
- Web の `/` は、workspace bootstrap / select route にする
- ここでは workspace を明示作成するか、既存 workspace を選択する
- 初回 `/` では workspace を自動作成しない
- `acp` launcher は no-arg chat 互換のため、loopback bearer client として
  `/api/v1/workspaces/bootstrap` を明示的に呼び、返された workspace を
  `chat --workspace <id>` に渡してよい。Web はこの endpoint を自動実行しない
- Web の canonical deep link は
  `/workspaces/{workspace_id}/sessions/{session_id}` を推奨する
...
owner-level serialization は lazy な `User` materialization と owner-scoped migration/backfill にだけ使います。

session create / restart と workspace delete blocker 判定は、workspace 状態の読取りと
session row の insert / transition を同じ workspace transaction で行います。
...
2. 既存 owner ごとに少なくとも 1 つの workspace を bootstrap / backfill し、既存 session をその workspace に結び付ける
    - 単一 upstream を前提にできる環境では operator-configured bootstrap upstream から workspace を作る
    - それができない環境では workspace-aware client を先に出し、workspace 作成後に session create を有効化する
3. owner-wide `GET /api/v1/sessions` を追加し、新規 session create は最初から
   `POST /api/v1/workspaces/{workspace_id}/sessions` に限定する
    - backfill の進め方
      - 既存 session row がある場合は、その owner の `owner_user_id` と bootstrap 済み `workspace_id` を使って
...
6. Web の `/` を workspace bootstrap へ寄せ、CLI に workspace command を追加する
...
1. Web の初回 `/` は workspace を自動作成せず、workspace の明示作成または選択を求める
...
3. backend 再起動後に live worker を失った session は、`closed` や `failed` へ落とさず
    `restartable` として扱う
4. phase 1 の session create は、`workspace.default_ref` に加えて session request ごとの
   pinned branch 名 override を受け付ける

### 12.2 残課題

1. phase 2 以降で、branch 名以外の pinned ref まで許可するか
    - phase 1 の pinned ref は branch 名だけを扱う
    - tag、commit SHA、または任意の完全修飾 ref まで広げるかは将来判断する
...
- 新規 session create は `POST /api/v1/workspaces/{workspace_id}/sessions` に揃える
...
- session create request ごとの pinned branch 名 override を受け付ける
...
- `/` を workspace bootstrap / select route にする
...
### 13.7 phase 2 以降へ残す保留事項

- pinned ref は、`workspace.default_ref` とは別に session create request ごとに渡す
  checkout target である
- phase 1 では branch 名だけを扱う
- tag、commit SHA、完全修飾 ref まで広げるかは phase 2 以降で決める
