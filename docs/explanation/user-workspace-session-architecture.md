...
phase 1 の deployment target は case-sensitive な local filesystem に限る。
起動時に mixed-case probe でその前提も検証する。
`checkout_relpath` を読む側は、検証済み `<state-root>` の dirfd から
`openat2(..., RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS)` を使う。
同等実装を採る場合は、component-wise `openat(..., O_NOFOLLOW)` で
単一 component ずつ解決する。
`/` からの string-based canonicalize を再実行する実装は許可しない。
解決に失敗した session は scrubbed error を残して `failed` へ進める。
checkout 作成 / cleanup / janitor の directory 操作も、同じ dirfd-based rule で辿る。
validated path string を別に開き直さない。
session tree は同じ local filesystem device 内に作る。
checkout 作成時は `fstat` で得た `st_dev` が state-root fd の `st_dev` と一致することも確認する。
不一致なら session を `failed` へ進める。
`WorkspaceFileAdapter` と terminal の working directory は、state-root 配下であっても
当該 session checkout root の dirfd に束縛する。
cross-session path への到達は拒否する。
terminal subprocess は kernel-enforced filesystem sandbox に入れる。
例としては Landlock や mount namespace である。
state-root の DB / events / 他 session checkout へは syscall レベルで到達できないようにする。
この sandbox では approved binary set だけを execute 可能にする。
raw system `git` は PATH 上にも絶対 path にも置かない。
approved binary set から `ln`、`install`、`git-lfs` などを除外する。
hardlink や Git extension 実行に使える tool は含めない。
checkout root 配下の任意 depth で、先頭 4 文字が `.git` の entry と `.git/` 配下は write 禁止にする。
...
4. child ごとの session-level lock 下で先に `checkout_relpath` を `NULL` に tombstone する。
   その後、workspace cleaner を disk cleanup の唯一の owner にする。
   lock の外で workspace directory tree 全体の recursive cleanup を実行する
...
- clone / fetch / push 実行時は `GIT_TERMINAL_PROMPT=0` を基本にする。
  ...
  明示注入した `GIT_CONFIG_*` など必要最小限の closed allowlist だけとする。
...
- phase 1 では repo-controlled な Git execution path を許可しない。
  例として `include.*`、filter / smudge / clean driver、`diff.*.command` / `textconv`、
...
- durable metadata は `active` の中でも mid-turn を識別できるように
  `in_flight_turn_id` 相当の情報を保持する。
  crash recovery は `active -> restartable` の前に、その turn を `interrupted` として event store へ seal する。
  `Busy` と `Canceling` の投影は、この reconciliation を経由してから
  `restartable` / `closing` / `deleting` へ分岐する
...
- ここでいう intact checkout の最低条件は、`.git/HEAD` が `checkout_commit_sha` へ解決できることである。
  その commit object が存在し、軽量な connectivity check を通ることも要る。
  checkout root の `st_dev` が state-root fd と一致することも要る。
  単なる directory 存在だけでは足りない
...
- credential 再検証や worker 起動が失敗した場合、その試行で取得した credential lease は revoke する。
...
- `deleting` workspace 配下の session に対する janitor の best-effort cleanup は disk 操作をせず、
...
default workspace binding、race 制御の正本は parent architecture の 8.3 を参照する。
...
この hierarchy 導入で重要なのは、`Workspace` を durable metadata として分離することである。
`Session` は runtime 単位とし、`WorkspaceFileAdapter` は session checkout root の file boundary として分離する。

特に session create / restart では `WorkspaceStorePort`、`WorkspaceCheckoutPort`、
`SessionStateStorePort`、`WorkerLifecyclePort` を組み合わせる。
workspace 所有情報の解決、checkout materialization、runtime metadata 保存、worker 制御は混ぜずに扱う。
...
owner-level serialization は `users` row や default binding を触る操作にだけ使う。
例は compatibility create、`PUT /api/v1/users/me/default-workspace`、workspace delete step 1 である。
